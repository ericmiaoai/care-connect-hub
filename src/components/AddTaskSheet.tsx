...
interface AddTaskSheetProps {
  ...
  defaultDate?: string;   // YYYY-MM-DD pre-fill; blank = unscheduled
  isOnline?:   boolean;
  onSave:      (title: string, dueDate: string | null, priority: UITask["priority"]) => Promise<{ error: string | null }>;
}

export function AddTaskSheet({
  open, onOpenChange, defaultDate = "", isOnline = true, onSave,
}: AddTaskSheetProps) {
  ...
  const handleSave = async () => {
    ...
    const dueDate: string | null = date
      ...
      : null;
    const { error } = await onSave(title.trim(), dueDate, priority);
    setSubmitting(false);
